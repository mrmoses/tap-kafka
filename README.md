...
| auto_offset_reset         | String  |           | (Default: latest) KafkaConsumer setting. What to do when the consumer's seek offset is out of range for the topic — e.g. a deleted-and-recreated topic whose offsets reset to 0. `latest` resumes at the partition's high watermark (current behavior; skips the recreated backlog); `earliest` restarts from offset 0 and drains it. One of: `earliest`, `latest`. |
